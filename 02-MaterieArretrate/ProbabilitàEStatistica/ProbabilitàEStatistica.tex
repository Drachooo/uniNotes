...
\usepackage{listings}
\definecolor{keywordcolor}{rgb}{0.1, 0.1, 0.8} % Un blu scuro per le parole chiave
\definecolor{backcolor}{rgb}{0.95, 0.95, 0.92} % Un grigio/crema chiarissimo per lo sfondo
% Configurazione base per rendere R leggibile
\lstset{
  language=R,
  ...
  % Definiamo il commento usando l'istruzione 'morecomment' 
  % ma passandola in modo che non venga interpretata come parametro
  morecomment=[l]{\#}, 
...
\begin{document}
    ...
    \begin{center}
      ...
      \Definizione{probabilità classica}{
        ...
        \begin{equation*}
          P_{classica} = \frac{n\textsuperscript{o} casi favorevoli}{n\textsuperscript{o} casi possibili}
        \end{equation*}
      }

      Questa definizione di probabilità vale solo quando gli eventi sono equiprobabili e, inoltre, presuppone un numero finiti di casi possibili.
...
\begin{description}
  \item[\textbf{ggplot2:}] per la visualizzazione dei dati 
  \item[\textbf{dplyr:}] per la manipolazionedei dati, inclusi filtraggio, raggruppamento e sintesi dei dati
\item[\textbf{tidyr:}] per pulire e ristrutturare i dati
\item[\textbf{caret:}] per il machine learning
\item[\textbf{lubridate:}] per lavorarecon date e orari
\item[\textbf{stringr:}] per lavorarecon le stringhe, incluse funzioni per il pattern matching, la manipolazionedelle stringhe e le espressioni regolari
\item[\textbf{reshape2:}] per trasformarei dati tra formati “wide” e “long”
\item[\textbf{data.table:}]per la manipolazione e aggregazione dei dati, particolarmente utile per lavorare
...
?nome\_funzione o ?nome\_pacchetto e premi Enter
...
\vario{Nota}{Usando la sintassi ??"search term" ottieni una lista di tutte le pagine di aiuto che contengono il termine}
...
\begin{itemize}
  \item concatenando vettori come colonne (funzione cbind)
  \item concatenando vettori come righe (funzione rbind)
  \item usando la funzione matrix(), specificando numero di righe e colonne
\end{itemize}
...
Le operazioni tra matrici seguono le regole dell'algebra lineare (cioè la dimnesione e la forma necessarie degli input in relazione tra loro dipendono dall'operazione). L'operatore \%*\% esegue la moltiplicazione tra matrici, mentre la funzione \textbf{t()} calcola la trasposta
