...
\begin{itemize}
  \item Vettori $\rightarrow$ una dimensione
  \item Matrici $\rightarrow$ due dimensioni
  \item Array $\rightarrow$ più dimensioni
\end{itemize}
...
Un data frame è una struttura di dati bidimensionale che permette di memorizzare dati in forma tabellare.
I data frame hanno righe e colonne, e ogni colonna può essere un vettore diverso. I vettori possono anche avere tipi di dati differenti.
...
\section{Fattori}
Un factor è una struttura di dati utilizzata per lavorare con dati categorizzabili.
...
\begin{codiceR}
  ifelse(condizione, yes = ..., no = ...)
   ...
   #Esempio
  > voti <- c(6,9,10)
  iflese(voti > 8, yes = "Ottimo", no = "Sufficiente")
...
\begin{itemize}
  \item .RDATA è usato per salvare uno o più oggetti in un unico file. \begin{itemize}
    \item Salvare: \texttt{save()}
    \item Caricare: \texttt{load()}
  \end{itemize}
  \item .RDS è un formato usato per salvare un singolo oggetto R in un file
  \begin{itemize}
...
\begin{itemize}
  \item Scropt: è un file di testo contenente una sequenza di comandi R che si desidera seguire. Oer eseguirli si utiziza la funzione \texttt{source()}
  \item Funzione: è un blocco di codice che svolge un compito specifico e può essere irchiamato da altre parti del codice. Le funzioni in R si definiscono usando \texttt{function()}
