...
for r = 0:255
    ...
    % Gestione del clipping (valori fuori dal range rmin-rmax)
    if s < 0
        s = 0;
    elseif s > 255
        s = 255;
    ...
    % Inserimento nella LUT (Matlab indice da 1 a 256)
    lut(r + 1) = uint8(round(s));
end

%% 4. Applicazione della LUT e Visualizzazione Risultato
I_stretched = intlut(I, lut);

subplot(2,2,3); imshow(I_stretched); title('Dopo Stretching');
subplot(2,2,4); imhist(I_stretched); title('Istogramma Stretched');
