...
\begin{document}
  ...
  \begin{itemize}
    \item \textbf{TOP-DOWN:}
    \begin{itemize}
      \item Fase 1: considerare le specifiche GLOBALMENTE e produrre uno schema iniziale completo ma con POCHI CONCETTI molto ASTRATTI
      \item Fase 2: RAFFINARE i concetti astratti fino ad arrivare allo schema concettuale COMPLETO in ogni DETTAGLLIO
    \end{itemize}
    \item \textbf{BOTTOM-UP:}
    \begin{itemize}
      \item Fase 1: DECOMPORRE le specifiche iniziali in PARTI ELEMENTARI (frasi che descrivono lo stesso concetto)
      \item Fase 2: GENERARE gli SCHEMI per tutte le parti elementari individuate.
      \item Fase 3: FONDERE gli schemi (introducendo altri costrutti del modello E-R) in modo da INTEGRARE tutti gli schemi componenti e generare lo SCHEMA FINALE.
    \end{itemize}
    \item \text{INSIDE-OUT:}
    \begin{itemize}
      \item Fase 1: INDIVIDUARE nelle specifiche alcuni CONCETTI IMPORTANTI (detti concetti GUIDA)
      \item Fase 2: GENERARE gli SCHEMI per i concetti GUIDA
      \item Fase 3: FONDERE gli schemi precedenti e generare lo SCHEMA FINALE
    \end{itemize}
  \end{itemize}

  \vspace{1em}
  \textbf{\large Analisi di qualità dello schema:}
  \begin{description}
    ...
    \item[Verifica di leggibilità:] quando rappreenta i requisiti in maniera naturale facilmente comprensibile. Attenzione ai nomi (di entità, relazioni e attributi), e alle rappresentazioni contorte.  
  \end{description}

  Uno schema concettuale \textbf{non minimale} contiene ridondanze. \\
  Casi di ridondanza: 
  \begin{itemize}
    \item Ereditarietà delle Relazioni
    \item Cicli di Relazioni
  \end{itemize}

Da inserire le foto dell'ereditarietà e dei cicli di Relazioni
...
\text{Semplificazioni dello schema}
...
\begin{itemize}
  \item Trasformazione di una relazione ternaria in una entità(sempre applicabile)
  \item riduzione di una relazione ternaria a due relazioni binarie (PRECONDIZIONE: una delle entità partecipa alla relazione ternaria con cardinalità(1,1))
\end{itemize}

Eliminazione delle relazioni ternarie iMmagine
