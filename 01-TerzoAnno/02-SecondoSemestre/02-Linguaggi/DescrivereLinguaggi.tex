...
\begin{mydef}{stato}
    ...
    \begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
        ...
        \item \textbf{memoria} (store): insieme degli effetti seugli identificatori
    \end{itemize}
\end{mydef}
...
\begin{mydef}{comandi}
    Le \textbf{comandi} denotano richieste di trasformazioni/modifiche della memoria.
\end{mydef}

I comandi, quindi, rappresentano funzioni di trasformazione (da stato a stato) e devono essee eseguiti per poter attuare la trasformazione (irreversibile) della memoria.
...
\vario{Reversibile}{
    \textbf{Reversibile} = vale eclusivamente nel raggio di azione (scope) dell'identificate.
}
...
\vario{Principio di induzione (matematica)}{
    ...
    Per eseplicitare il vantaggio di avere $\mathrm{A}$ ben fondato, seperiamo la dimostrazione sugli elementi minimali (base):
    \[
...
Un algoritmo è un'entià astratta concretizzata in un programma, ma non ci interessa sapere se il programma implementa esattamente l'algoritmo, ma ci interessa che il programma implementi la funzione descritta dall'algoritmo.\\[0.2ex]
...
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
    ...
    \item \textbf{semantica operazionale}: descrive trasformazioni di stato (guarda come i risultato finali vengono prodotti)
...
\[
    \mathcal{E}: \mathrm{Prog} \rightarrow ((\mathrm{Var} \rightarrow \mathrm{Val}) \rightarrow (\mathrm{Var} \rightarrow \mathrm{Val})) 
\]
...
\begin{mydef}{semantica assiomatica}
    La \textbf{semantica assiomatica} è un modello matematico dei programmi basto sulla logica formale (calcolo dei predicati). 
\end{mydef}
