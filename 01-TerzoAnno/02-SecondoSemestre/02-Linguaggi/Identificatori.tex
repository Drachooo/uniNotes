...
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
    ...
    \]
    \item \textbf{esitono parole riservate?} (parole già associate a specifici significati)
...
\begin{mydef}{occorrenza di legame}
    L'\textbf{occorrenza di legame} (o \textbf{definizione}) crea il legame tra nome e il suo significato.
\end{mydef}

\begin{mydef}{occorrenza di applicazione}
    L'\textbf{occorrenza di applicazione} usa il nome per accedere al suo significato.
\end{mydef}

\begin{mydef}{occorrenza libera}
    L'\textbf{occorrenza libera} utilizza un nome a cui non è stato associato nessun significato.
\end{mydef}
...
\hl{Distingue i binding rispetto alla durata del legame e alla dinamicità del legame}.
...
\hl{Distingue i binding rispetto al momento in cui il binding viene creato}.
...
\begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    ...
    \item $\mathcal{D} \mathrm{Val} = \mathcal{N} \cup \mathcal{B}$: insieme dei valori numerici e booleani (valori denotabili)
\end{itemize}
...
\begin{mydef}{ambiente}
    ...
    \[
        \mathrm{Env} = \bigcup\limits_{\mathrm{V} \ \subseteq_{f} \ \mathrm{Id}} \mathrm{Env_V}
    \]
...
Data un'espressione o una frase scritta in un certo linguaggio, gli identificatori liberi sono quelli che non hanno nessuna dichiarazione che li coinvolge.\\[0.2ex]
...
\begin{mydef}{identificatore libero}
    La funzione:
    \[
        \mathbf{FI} : \mathbf{Exp} \to \mathbf{Id}
    \]
...
Così come possiamo definire gli indentificatori liberi, possiamo definire anche anche quelli definiti $\mathbf{DI}$ (sempre per induzione sulla struttura sintattica delle espressioni).\\[0.2ex]
La definizione è molto semplice in quanto nelle espressioni non abbiamo costrutti che creano legami (occorrenze), ma solo costrutti che utilizzano occorrenze.\\[0.2ex]
...
\[
    \rho \vdash \mathbf{e} \to \mathbf{e'}
\]

Ciò significa che l'espressione $\mathrm{e}$ viene valutata in $\mathrm{e}'$ nell'ambiente $\rho$.
...
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
    ...
    \vario{Regola $\mathcal{E}_1$ (assioma)}{
        \[
            \mathcal{E}_1: \ \rho \vdash \mathrm{m} \texttt{ op } \mathrm{n} \to \mathrm{p} \quad \Leftrightarrow \quad \mathrm{m} \texttt{ op } \mathrm{n} = \mathrm{p}
        \]
    }
    \item \textbf{regola 2} (assioma):
    \vario{Regola $\mathcal{E}_2$ (assioma)}{
        \[
           \mathcal{E}_2: \ \rho \vdash \mathrm{I} \to_\mathrm{e} \mathrm{n} \quad \Leftrightarrow \quad \rho(\mathrm{I}) = \mathrm{n}
        \]
    }
    \item \textbf{regola 3}:
    \vario{Regola $\mathcal{E}_3$}{
        \[
           \mathcal{E}_3:\;
           \begin{prooftree}
            \hypo{\rho \vdash \mathrm{e} \to_\mathrm{e} \mathrm{e}'}
            \infer1{\rho \vdash \mathrm{e} \texttt{ op } \mathrm{e}_0 \to_\mathrm{e} \mathrm{e}' \texttt{ op } \mathrm{e}_0}
           \end{prooftree}
        \]
    }
    \item \textbf{regola 4}:
    \vario{Regola $\mathcal{E}_4$}{
        \[
           ...
           \begin{prooftree}
            \hypo{\rho \vdash \mathrm{e} \to_\mathrm{e} \mathrm{e}'}
            \infer1{\rho \vdash \mathrm{m} \texttt{ op } \mathrm{e} \to_\mathrm{e} \mathrm{m} \texttt{ op } \mathrm{e}'}
           \end{prooftree}
        \]
    }
    \item \textbf{regola 5}:
    \vario{Regola $\mathcal{E}_5$}{
        \[
           ...
           \begin{prooftree}
            \hypo{\rho \vdash \mathrm{e} \to_\mathrm{e} \mathrm{e}'}
            \infer1{\rho \vdash \texttt{not } \mathrm{e} \to_\mathrm{e} \texttt{not } \mathrm{e}'}
           \end{prooftree}
        \]
    }
    \item \textbf{regola 6}:
    \vario{Regola $\mathcal{E}_6$}{
        \[
           \mathcal{E}_6: \ \rho \vdash \texttt{not } \mathrm{t} \to \mathrm{t}' = \texttt{not } \mathrm{t}
        \]
    }
    \item \textbf{regola 7}: valuta un identificatore $\mathrm{x}$ nell'ambiente $\rho$
    \vario{Regola $\mathcal{E}_7$}{
        \[
           \mathcal{E}_7: \ \rho \vdash \mathrm{x} \to \mathrm{k} \quad \Leftrightarrow \quad \rho(\mathrm{x}) = \mathrm{k}
        \]
    }
\end{itemize}
...
\begin{mydef}{Tipo}
    Un \textbf{tipo} determina l'insieme di valori omogenei (condividono una certa proprietà strutturale) dotato di un insieme di operazioni definite per i valori di quel tipo.
\end{mydef}
...
\begin{minipage}[t]{0.45\textwidth}
    \vario{Assioma}{
        \[
            \mathcal{E}_{\mathbf{S_i}}: \ \textcolor{red}{\vdash} \ \mathbf{e}: \tau
        \]
...
\begin{minipage}[t]{0.45\textwidth}
    \vario{Regola}{
        \[
            \mathcal{E}_{\mathbf{S_i}}: \ \textcolor{red}{\Delta \vdash_\mathbf{V}} \ \mathbf{e}: \tau
        \]
...
\begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
    ...
    \begin{minipage}[t]{0.45\textwidth}
        \vario{Regola $\mathcal{E}_{\mathrm{S_1}}$ (assioma)}{
            \[
    ...
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        \vario{Regola $\mathcal{E}_{\mathrm{S_2}}$ (assioma)}{
            \[
...
È chiaro che possiamo aspettarci che i due ambienti definiti sullo stesso programma parlino in modo coerente degli stessi identificatori.
