...
\subsection{Guasti}
Una transazione T sceglie in modo atomico l'esito di $\verb|commit|$ nel momento in cui scrive nel file di log in modo sincrono (primitiva $\verb|force|$) il suo record di $\verb|commit| \ \mathrm{C(T)}$. 
...
Il \textcolor{azzurrino}{\textbf{gestore dei metodi di accesso}} esegue il piano di esecuzione prodotto dall'ottimizzatore e produce sequenze richieste di accessi ai blocchi della base di dati presenti in memoria secondaria.\\[0.2ex]
...
Ad ogni file viene assegnato un numero n di blocchi contigui e ciascun blocco è dotato di un numero m di posizioni disponibili per le tuple (quindi è un array bidimensionale $\mathrm{n \times m}$).\\[0.2ex]
...
\begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
  ...
  Ogni record dell’indice primario contiene una coppia $\mathrm{\langle v_i, p_i\rangle}$, dove:
  \begin{itemize}
  ...
  \begin{itemize}[label=$\diamond$, itemsep=\medskipamount]
    ...
    \begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
      \item indice è denso $\mathrm{K} \rightarrow$ è presente nell'indice
      \begin{enumerate}[itemsep=\smallskipamount]
      ...
      \item indice è sparso $\mathrm{K \rightarrow}$ potrebbe non essere presente nell'indice
      \begin{enumerate}[itemsep=\smallskipamount]
        \item ccansione sequenziale dell'indice fino al record $\mathrm{(K", p_k)}$, dove $\mathrm{K"}$ è il valore più grande che sia minore o uguale a $\mathrm{K}$
  ...
  \begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
    ...
    Nel caso in cui il blocco sopravvive, va sostituito $\mathrm{K}$ nel record dell'indice con il primo valore $K"$ presente nel blocco.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item $\mathrm{v_i}$: valore della chiave di ricerca
  \item $\mathrm{p_i}$ è il puntatore al bucket di puntatori che individuano nel file sequenziale tutte le tuple con chiave vi
\end{itemize}
...
\begin{itemize}[label=$\diamond$, itemsep=\medskipamount]
  \item \textbf{ricerca} di una tupla con chiave di ricerca $\mathrm{K}$
  \begin{enumerate}[itemsep=\smallskipamount]
    \item scansione sequenziale dell'indice per trovare il record $\mathrm{(K, p_k)}$
...
Per migliorare le prestazioni degli accessi a un indice di grandi dimensioni, si utilizza il il $\verb|B+-tree|$, in cui:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  ...
  \item insierimenti e cancellazioni non alterano le prestazioni dell'accesso ai dati: l'albero si mantiene bilanciato
\end{itemize}
...
\begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
  ...
  Può contenere fino a $\mathrm{n - 1}$ valori ordinati di chiave di ricerca e fino a $\mathrm{n}$ puntatori ai dati.\\[0.2ex]
...
Il fan-out è un parametro che fornisce il limite massimo, ma bisogna anche fornire un limite minimo per evitare che i nodi siano troppo vuoti e sono chimati \textbf{vincoli di riempimento}:
\begin{itemize}[label=$\diamond$, itemsep=\medskipamount]
  \item \textbf{vincoli per i nodi foglia}: ogni nodo foglia può contenere un numero di valori di chiavi ($\mathrm{\# \ chiavi}$) vincolato come:
  \begin{equation*}
    \mathrm{\left\lceil \frac{n - 1}{2} \right\rceil \leq \# \ chiavi \leq n - 1}
  \end{equation*}
  \item \textbf{vincoli per i nodi intermedi}: ogni nodo intermedio può contenere un numero di puntatori ($\mathrm{\# \ puntatori}$) vincolato come:
  \begin{equation*}
...
\esempio{utilizzo del \texttt{B+-tree}}{
  consideriamo un \texttt{B+-tree} con fan-out = 4 conetente:
  \[
...
Per effettuare la ricerca di una tupla con chiave di ricerca K si efettuano i seguenti passi:
\begin{enumerate}[itemsep=\medskipamount]
  \item \textbf{cercare nel nodo radice il più piccolo valore di chiave maggiore di K}
  \begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
    \item se il valore esiste ($\mathrm{value = k_i}$) $\rightarrow$ sesguire il puntatore $\mathrm{p_i}$
    \begin{figure}[H]
      \centering
...
Il costo della ricerca dipende dalla profondità della'lbero in termini di \textbf{accessi alla memoria secondaria}, dove ogni metodo rappresenta un blocco.\\[0.2ex]
Il costo in una struttura ad alberp è pari alla profondità dell'albero, che nel $\verb|B+-tree|$ è indipendente dal percorso ed è funzione del fan-out $\mathrm{n}$ e del numero di valori di chiave presente nell'albero $\mathrm{\# \ valoriChiave}$.
\begin{equation*}
...
\begin{enumerate}[itemsep=\medskipamount]
  ...
  \begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
    ...
    \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
      ...
      \item se è \textbf{indice secondario} $\rightarrow$ si aggiornnao i bucket dei puntatori
    \end{itemize}
...
\esempio{plit di un nodo foglia}{
...
\begin{enumerate}[itemsep=\medskipamount]
  ...
  \begin{enumerate}[itemsep=\smallskipamount]
    ...
    \begin{itemize}[itemsep=\smallskipamount]
      \item se non cè un nodo foglia fratello che unito rispetta il vincolo di riempimento massimo, si fa prima il merge e poi lo split
...
Si potrebbe utilizzare un array indicizzato dalla ciave, ma questo non è pratico perchè i valori della chiave sono tanti, quindi, si utilizza una \textbf{funzione di hash} che mappa i valori di chiave in indirizzi di memorizzazione delle tuple:
\begin{equation*}
  \mathrm{h : K \rightarrow B}
\end{equation*}

dove:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  \item K: dominio delle chiavi
  \item B: dominio degli indirizzi di memorizzazione
\end{itemize}
...
\definizione{fattore riempimento}{
  Il \textbf{fattore riempimento} definisce la frazione di spazio fisico mediamente utilizzata da un blocco.
}

Siano:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  \item T: numero di tuple
  \item F: fattore blocco (numero di tuple che ci stanno in un blocco)
...
\paragraph{Operazione: ricerca con chiave K}
Per efettuare la ricerca di una tupla con chiave di ricerca K si efettuano i seguenti passi:
\begin{enumerate}[itemsep=\medskipamount]
  \item calcolare $\mathrm{b = h(f(K))}$ (costo zero)
  \item accendere al bucket $\mathrm{b}$ (costo: 1 accesso alla pagina di memoria secondaria)
  \item accendere alle $\mathrm{n}$ tupe attraverso i puntatori del bucket (costo: $\mathrm{m}$ accessi a pagine di memoria secondaria, con $\mathrm{m \leq n}$)
\end{enumerate}
...
La probabilità che uno stesso bucket riceva $\mathrm{t}$ chiavi su $\mathrm{n}$ insierimenti è:
\begin{equation*}
...
dove:
\begin{itemize}[label=$\triangleright$]
  \item B: numero totale di bucket
\end{itemize}
