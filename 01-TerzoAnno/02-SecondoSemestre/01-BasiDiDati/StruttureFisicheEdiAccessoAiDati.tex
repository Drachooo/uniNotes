...
\begin{itemize}[label=$\Rightarrow$]
  \item \textbf{effetto della modifica}: viene liberata la pagina "dirty" e il bit di stato J viene posto a 0
\end{itemize}
...
\begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
  ...
  \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    ...
    \item $\boldsymbol{\textbf{I(T,O,AS)}}$: \textcolor{azzurrino}{\textbf{insert}} di un oggetto $\mathrm{O}$ da parte di una transazione $\mathrm{T}$ con il suo after state $\mathrm{AS}$ (stato dell'oggetto dopo l'operazione)
...
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  ...
  L'insert/delete viene disfatto cancellando/inserendo $\mathrm{O}$
...
In caso di \textbf{tuple di lunghezza fissa}, il dizionario non è necessario: si memorizza la dimensione delle tuple e l'offset del punto iniziale.\\[0.2ex]
...
Ad ogni file viene assegnato un numero $\boldsymbol{n}$ di blocchi contigui e ciascun blocco è dotato di un numero m di posizioni disponibili per le tuple (quindi è un array bidimensionale $\mathrm{n \times m}$).\\[0.2ex]
Ogni tupla è dotata di un valore numerico i che rappresenta l'indice della tupla in i-esima posizione nell'array.

\begin{itemize}[label=$\Rightarrow$, itemsep=\smallskipamount]
  \item \textbf{inserimento}
  \begin{itemize}[itemsep=\smallskipamount]
    \item Iniziale: l’indice $\boldsymbol{i}$ viene ottenuto incrementando un contatore
...
\begin{itemize}[label=$\triangleright$, itemsep=\medskipamount]
  ...
  Può contenere fino a $\boldsymbol{n - 1}$ valori ordinati di chiave di ricerca e fino a $\boldsymbol{n}$ puntatori ai dati.\\[0.2ex]
...
\begin{enumerate}[itemsep=\medskipamount]
  ...
  \item accedere alle $\mathrm{n}$ tupe attraverso i puntatori del bucket (costo: $\mathrm{m}$ accessi a pagine di memoria secondaria, con $\mathrm{m \leq n}$)
\end{enumerate}
