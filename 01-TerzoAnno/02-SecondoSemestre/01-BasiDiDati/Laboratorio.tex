%!TeX root = BasiDiDati.tex
  ...
  \chapter{Laboratorio}
  PostgrSQL è un \textcolor{azzurrino}{\textbf{R}}elational \textcolor{azzurrino}{\textbf{D}}ata \textcolor{azzurrino}{\textbf{B}}ase \textcolor{azzurrino}{\textbf{M}}anagement \textcolor{azzurrino}{\textbf{S}}ystem, con alcune funzionalità orientate agli oggetti.
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/ClientServer.png}
  \end{figure}
  ...
  \vario{Nota}{
    Gli identificatori \textbf{sono case sensitive}! : \texttt{idPersona} e \texttt{idpersona} rappresentano un solo identificatore
  }
  ...
  \begin{esempiobox}{Comando \texttt{CRETATE DOMAIN}}
    \begin{sqlcode}
      ...
      CREARE DOMAIN Voto AS SMALLINT DEFAULT NULL
  ...
  \begin{itemize}[label=$\diamond$, itemsep=\medskipamount]
    ...
    \end{sqlcode}
    \item $\verb|PRIMARY KEY|$: chiave primaria (una sola, implica NOT NULL) 
    \begin{sqlcode}
  ...
  Un vincolo di integrità referenziale (interrelazionale) crea un legame tra i valori di un attributo (o di un insieme di attributi) A della tabella corrente (interna/slave) e i valori di un attributo (o di un insieme di attributi) B di un'altra tabella (esterna/master).\\[0.2ex]
  ...
  \begin{itemize}[itemsep=\smallskipamount]
    ...
    \item  l'attributo B della tabella esterna deve essere soggetto a un vincolo UNIQUE (o PRIMARY KEY).
  \end{itemize}
...
Lavora su una singola riga alla volta. Se la tua tabella ha 100 righe, il costrutto \texttt{SELECT titolo || ' - ' || città} calcolerà e restituirà 100 risultati testuali separati, uno per ogni riga.

\vario{nota}{Non è un'esclusiva di SELECT, può infatti essere utilizzato anche nella clausola WHERE e nella clausola ORDER BY}


\textbf{\large Concatenazione in clausola WHERE (per filtrare i dati)}
\begin{codiceSQL}
  -- Trova la riga in cui l'unione di nome e cognome da esattamente 'Mario Rossi'
WHERE nome || ' ' || cognome = 'Mario Rossi'
\end{codiceSQL}

\textbf{\large Concatenazione in clausola ORDER BY (per ordinare i risultati)}
\begin{codiceSQL}
...
È il più restrittivo: nessuna anomalia è permessa. Se si usa questo livello, si deve prevedere la possibilità di transazioni abortite
